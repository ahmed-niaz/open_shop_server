...
import { PRODUCT_REPOSITORY, ProductRepositoryPort } from "../../ports/product.repository.port.js";
...
export class GetProductHandler implements IQueryHandler<GetProductQuery, Product> {
    ...
    async execute(query: GetProductQuery): Promise<Product> {
        ...
        const product = await this.productRepository.findById(new ProudctId(query.id));
        
        if(!product) {
            throw new ApplicationException(`Product with ID ${query.id} not found`, ApplicationExceptionCode.NOT_FOUND);
        }
        return product;
    }
  }