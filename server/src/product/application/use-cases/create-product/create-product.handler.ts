...
export class CreateProductHandler implements ICommandHandler<CreateProductCommand, void> {
  ...
  async execute(command: CreateProductCommand): Promise<void> {
    const existingProductBySku = await this.productRepository.findBySku(SKU.create(command.sku));

    if (existingProductBySku) {
      throw new ApplicationException(`Product with SKU ${command.sku} already exists`, ApplicationExceptionCode.CONFLICT);
    }

    const existingProductByName = await this.productRepository.findByName(command.name);
